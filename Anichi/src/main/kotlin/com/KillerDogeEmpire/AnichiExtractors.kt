...
import com.RowdyAvocado.AnichiParser.AnichiVideoApiResponse
import com.RowdyAvocado.AnichiParser.LinksQuery
import com.RowdyAvocado.AnichiUtils.embedIsBlacklisted
import com.RowdyAvocado.AnichiUtils.fixSourceUrls
import com.RowdyAvocado.AnichiUtils.fixUrlPath
import com.RowdyAvocado.AnichiUtils.getHost
import com.RowdyAvocado.AnichiUtils.getM3u8Qualities
...
object AnichiExtractors : Anichi() {
    ...
    suspend fun invokeInternalSources(
        ...
        apiResponse.data?.episode?.sourceUrls?.apmap { source ->
            safeApiCall {
                val link =
                        fixSourceUrls(source.sourceUrl ?: return@safeApiCall, source.sourceName)
                                ?: return@safeApiCall
                if (URI(link).isAbsolute || link.startsWith("//")) {
                    ...
                    val links =
                            app.get(fixedLink).parsedSafe<AnichiVideoApiResponse>()?.links
                                    ?: emptyList()
                    links.forEach { server ->
                        val host = server.link.getHost()
                        when {
                            source.sourceName?.contains("Default") == true -> {
                                if (server.resolutionStr == "SUB" ||
                                                server.resolutionStr == "Alt vo_SUB"
                                ) {
                                    getM3u8Qualities(
                                                    server.link,
                            ...
                            server.hls != null && server.hls -> {
                                ...
                                server.subtitles?.map { sub ->
                                    subtitleCallback.invoke(
                                            SubtitleFile(
                                                    SubtitleHelper.fromTwoLettersToLanguage(
                                                            sub.lang ?: ""
                                                    )
                                                            ?: sub.lang ?: "",
